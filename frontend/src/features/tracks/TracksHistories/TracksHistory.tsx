...
const TracksHistory: React.FC = () => {
  ...
  return (
    <>
      ...
      <Grid item xs={12}>
        <Typography component="h3" variant="h5" marginY="10px" style={{ opacity: '0.5' }}>
          {trackHistory.length > 0 ? 'Your recent:' : 'You have not listened to any tracks yet!'}
        </Typography>

        {trackHistory.length === 0 && (
          <Button component={Link} variant="contained" to="/" sx={tracksHistoryClasses.backToHomeButton}>
            Back to Home
          </Button>
        )}
        ...
        <List style={{ width: '100%' }}>
          {trackHistory.map((listenedTrack) => (
            <ListItemButton key={listenedTrack._id} alignItems="center" sx={tracksHistoryClasses.listItemButton}>
              <ListItemText
                primary={
                  <Typography component="span" sx={tracksHistoryClasses.datetimeBlock}>
                    {listenedTrack.datetime.toString()}
                  </Typography>
                }
                secondary={
                  <Typography sx={tracksHistoryClasses.trackNameAndArtistBlock}>
              ...
              />
              <ListItemSecondaryAction style={{ opacity: '0.5' }}>{listenedTrack.track.trackDuration}</ListItemSecondaryAction>
            </ListItemButton>
          ))}
        </List>
      </Grid>
